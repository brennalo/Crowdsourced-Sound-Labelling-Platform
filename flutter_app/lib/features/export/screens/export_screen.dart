    ...
    final confirmed = await showDialog<bool>(
      ...
      builder: (_) => AlertDialog(
        ...
        actions: [
          ...
          FilledButton(onPressed: () => Navigator.pop(context, true), child: const Text('Retrain')),
        ],
      ),
    );
    ...
    return Scaffold(
      ...
      body: ListView(
        ...
        children: [
          ...
          // ── Past exports ──────────────────────────────────
          Text('Past Exports', style: theme.textTheme.labelLarge),
          ...
          exportsAsync.when(
            ...
            data: (exports) {
              if (exports.isEmpty) {
                return Text('No exports yet.',
                    style: TextStyle(color: theme.colorScheme.outline));
              }
              return Column(
                children: exports.map((j) => _ExportJobTile(
                      job: j,
    ...
    return Container(
      ...
      child: Column(
        ...
        children: [
          ...
          _StatRow(label: 'Added last 7 days', value: '+${stats.addedLast7Days}'),
          _StatRow(label: 'Consensus flips', value: '${stats.consensusFlips}'),
          _StatRow(label: 'Researcher corrections', value: '${stats.researcherCorrections}'),
...
class _LabelBar extends StatelessWidget {
  ...
  final int count;
  final int total;
  const _LabelBar({required this.label, required this.count, required this.total});
  ...
  Widget build(BuildContext context) {
    ...
    final color = isChainsaw ? theme.colorScheme.error : theme.colorScheme.primary;
...
class _ExportJobTile extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final (icon, color, text) = switch (job.status) {
      'done' => (Icons.check_circle_outline, Colors.green, 'Ready'),
